...
+ Osnovna komanda za paljenje je /Magit User Manual/-a =<C-x>g=, =G= (=<M-x>magit-refresh=) osvježuje pogled, a =q= ga zatvara
+ =h= (kada u Magitu) otvara popis dostupnih komandi a =hh= otvara /Help Pages/
* Suradnja s Vimom
