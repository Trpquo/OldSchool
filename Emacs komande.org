...
** Baratanje prozorima i panelima
  + =<Leader>b{}= je za /buffere/ (prozore, tabove...)
    ...
    + =<C-x><lijevo>= prebacuje na prethodni buffer, =<C-x><desno>= prebacuje na idući buffer
...
+ =<C-s>= (~I-search~) i =<C-r>= (? je u evilu za /undo/) tj. =/=, =f= i =t= pokreću pretragu za pojmom/znakom, a ponavljanje =<C-s/r>= dok j search buffer otvoren tj. =n= i =N= te =;= i =,= u normalnom modu skaču do idućeg/prethodnog pretraživanog pojma/znaka
