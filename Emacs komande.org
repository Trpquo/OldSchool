...
** Fetch (=f=) i Pull (=F=)
+
+ =hzz= je za ~stash~ (nešto kao lokalni /commit/, pa čak i pita za /message/), a /unstash/ se radi s =hzp= (/statsh pop/) ili sa =hza= + =hzk= (/statsh apply/ + /stash drop/)
  - ako zaglavim s /merge conflict/-om sa stashom, njega (koliko znam?) ne mogu forcati ili riješiti u /diff/-u, tada mogu /checkout/-ati stash, /pop/-ati ga (ili /apply/-ati), te nakon toga /pull/-ati zadnje izmjene i pokušati riješiti konflikte.
** Log (=l=)
