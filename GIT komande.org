...
- ~git fetch~ je komanda za ažuriranje /origin/<ime-grane> grana, ali na nikoji način ne utjeće na lokalne grane ili /priloge/. Ona samo ažurira na /remote/ grani predstavu /remote/ repozitorija.  
  - puni oblik komande je ~git fetch <remote> <ime-grane>~. Obično <remote> argument Git može zaključiti iz postavki koju udaljenu granu aktivna grana "prati" (/tracks/) (a postavljenu sa ~git branch -u <remote>~. Argument <ime-grane> se odnosi na ime grana (lokalne i udaljene) koje se želi sinkronizirati (premda se tu može staviti bilokakav Git-u razumljiv identifikator lokacije). Dakle, komanda se čita: /Odi na granu <grana> na <remote> adresi i kopiraj sve razlikovne priloge na istoimenu granu na lokalnu <remote> prateću granu./ Pored toga, /<ime-grane>/ se može doraditi sa /refspec/ dvotočkom - /<source>/ ~:~ /<destinacija>/ - (npr. ~git fetch origin main:foo~, koja će dohvatiti nove /priloge/ na /main/ grani na udaljenom poslužitelju i prepisati ih na lokalnu /foo/ granu - ! ne na /origin/bar/ granu). Sa /refspec/ dvotočkom se može dohvatiti i pojedini /prilog/ sa udaljenog repozitorija poput /cherry-pick/-a (~git fetch origin <hash>:<lokalna-grana>~ će prepisati /prilog <hash>/ na /<lokalnu-granu>/; po potrebi će ju kreirati).
  - bez dodatnih argumenata, ~git fetch~ će dohvatiti sve sa svih grana udaljenog repozitorija.
  - dohvaćanje ničega sa udaljenog servera (~git fetch origin :foo~), shvatit će se kao želja za stvaranjem nove prazne grane u lokalnom repozitoriju (istovjetno ~git branch foo~)
*** pull
- ~git pull~ je kraći način za zadati ~git fetch <remote/grana>; git merge <remote/grana>~, zbog toga je prilično bitno (*!!!*) sa koje se aktivne grane komand /pull/ izvrti (što nije nužno istina za /fetch/ ili /push/ komande)
  * ~--rebase~ je oznaka za vrćenje drugih dviju komandi: (~git fetch; git rebase <remote/ime-grane>~).
    - ~git pull~ prima iste argumente kao i ~git fetch~ i oni se proslijeđuju /fetch/-u sa istim ulogama kako i očekivano, te se na to nadovezuje /merge/ ili /rebase/ komanda.
...
- ~git push~ je komanda za "objavljivanje" lokalnih izmjena na /remote/ repozitorij. Ona automatski ažurira i izgled /origin/ grana.
  - puni oblik komande je ~git push <remote> <ime-grane>~. Obično <remote> argument Git može zaključiti iz postavki koju udaljenu granu aktivna grana "prati" (/tracks/) (a postavljenu sa ~git branch -u <remote>~. Argument <ime-grane> se odnosi na ime grana (lokalne i udaljene) koje se želi sinkronizirati (premda se tu može staviti bilokakav Git-u razumljiv identifikator lokacije). Dakle, komanda se čita: /Odi na granu <grana> lokalno i kopiraj sve razlikovne priloge na istoimenu granu na lokalnu <remote> prateću granu i onda tu granu prepiši na <remote> repozitoriju./ Uzgred, kako dva argumenta daju sve potrebne informacije komandi, nije važno koja je grana trenutno aktivna u trenutku izvršenja komande.
    - ako se želi mijenjati odredište na udaljenom repozitoriju, može se koristiti sintaksa sa ~:~ (tzv. /colon refspec/), npr. ~git push origin foo^:bar~, gdje će se svi osim zadnjeg priloga na lokalnoj grani /foo/ prepisati na udaljenu granu /bar/; ili ~git push origin main:novaGrana~, gdje će se svi novi prilozi na /main/ grani prepisati na lokalni i udaljeni repozitorij na novu granu /origin/novaGrana/, odnosno na udaljenom serveru /novaGrana/.
    - :) /refspec/ dvotočka se može upotrijebiti i za brisanje grana na udaljenom repozitoriju (~git push origin :<grana-za-izbrisati>~); slanje kao /source/ "ništo" na /<destination>/ granu
shvatit će se da se ta grana na udaljenom poslužitelju želi učiniti ništetnom.
